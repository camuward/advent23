...
fn parse_hand(hand: &[u8], bid: u16) -> Hand {
    assert_eq!(hand.len(), 5);

    let cards = {
        let mut buf = [0; 5];
        let mut cards = hand.into_iter().map(parse_card);
...
fn part_one(input: &str) -> u32 {
    let mut hands: Vec<Hand> = input
        ...
        .map(|(hand, bid)| parse_hand(hand, bid.parse::<u16>().expect("invalid bid")))
        .collect();
